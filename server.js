// server.js — Biblioteca Obsidiana Backend V2.4.6
...
const {
  ...
  // E-mail via Resend
  // Coloque a chave no Render em Environment:
  // RESEND_API_KEY=re_xxxxxxxxx
  RESEND_API_KEY,
  FROM_EMAIL = "Biblioteca Obsidiana <onboarding@resend.dev>",
  SUPPORT_WHATSAPP = "5584997012535"
} = process.env;
...
function simpleEmailHtml(title, lines){
  const safeTitle = escapeHtml(title);
  const body = (Array.isArray(lines) ? lines : [String(lines || "")])
    .map(line => `<p style="font-size:16px;line-height:1.55;margin:0 0 12px">${escapeHtml(line)}</p>`)
    .join("");

  return `
  <div style="font-family:Arial,sans-serif;background:#090914;color:#f7f2e6;padding:24px;border-radius:16px">
    <h1 style="color:#f6c94a;margin:0 0 16px">${safeTitle}</h1>
    ${body}
    <hr style="border:0;border-top:1px solid rgba(255,255,255,.18);margin:20px 0">
    <p style="color:#ff315f;font-weight:bold">Criado por Francisco Jerbesson de Freitas Morais e ChatGPT 5</p>
  </div>`;
}
...
async function sendEmail({ to, subject, text, html }){
  ...
  if(!response.ok){
    throw new Error(result?.message || result?.error || "Erro ao enviar e-mail pelo Resend.");
  }
...
app.get("/", (req, res) => res.json({ ok:true, name:"Biblioteca Obsidiana Backend V2.4.6" }));
app.get("/health", (req, res) => res.json({ ok:true, status:"online", version:"2.4.6" }));
...
/* =========================
   E-MAIL — TESTE E RECUPERAÇÃO DE SENHA
========================= */

app.post("/admin/test-email", requireAdmin, async (req, res) => {
  try{
    const to = normEmail(req.body.email || req.profile.email);
    ...
    await sendEmail({
      to,
      subject:"Teste de e-mail — Biblioteca Obsidiana",
      text:[
        "Olá!",
        "Este é um e-mail de teste da Biblioteca Obsidiana.",
        "Se você recebeu esta mensagem, o envio de e-mail está funcionando.",
        "Agora já podemos usar recuperação de senha por e-mail."
      ].join("\n\n"),
      html:simpleEmailHtml("Teste de e-mail", [
        "Olá!",
        "Este é um e-mail de teste da Biblioteca Obsidiana.",
        "Se você recebeu esta mensagem, o envio de e-mail está funcionando.",
        "Agora já podemos usar recuperação de senha por e-mail."
    ...
    res.json({ ok:true, message:"E-mail de teste enviado com sucesso.", email:to });
  }catch(error){
...
app.post("/forgot-password", async (req, res) => {
  try{
    ...
    const genericResponse = {
      ok:true,
      message:"Se este e-mail estiver cadastrado, enviaremos um código de recuperação."
    };
    ...
    const { error: insertError } = await supabase
      .from("bo_password_resets")
      .insert({
        email,
        code_hash: hashPin(code),
    ...
    await sendEmail({
      to:email,
      subject:"Código para redefinir sua senha — Biblioteca Obsidiana",
      text:[
        "Olá!",
        "Você pediu para redefinir sua senha da Biblioteca Obsidiana.",
        `Seu código é: ${code}`,
        "Esse código vale por 15 minutos.",
        "Se você não pediu isso, ignore este e-mail.",
        `Suporte WhatsApp: ${SUPPORT_WHATSAPP}`
      ].join("\n\n"),
      html:simpleEmailHtml("Redefinir senha", [
        "Olá!",
        "Você pediu para redefinir sua senha da Biblioteca Obsidiana.",
        `Seu código é: ${code}`,
        "Esse código vale por 15 minutos.",
    ...
    res.status(500).json({ ok:false, error:"Erro ao solicitar recuperação de senha." });
  }
});
...
app.post("/reset-password", async (req, res) => {
  try{
    ...
    if(!/^\d{6}$/.test(code)){
      return res.status(400).json({ ok:false, error:"Código inválido. Use os 6 números recebidos por e-mail." });
    }

    if(!validPin(newPin)){
      return res.status(400).json({ ok:false, error:"A nova senha precisa ter 4 números." });
    }
    ...
    if(!reset || reset.code_hash !== hashPin(code)){
      return res.status(400).json({ ok:false, error:"Código incorreto ou expirado." });
    }
    ...
    res.json({ ok:true, message:"Senha alterada com sucesso. Agora faça login com a nova senha." });
  }catch(error){
...
app.listen(port, () => console.log(`Biblioteca Obsidiana V2.4.6 online na porta ${port}`));