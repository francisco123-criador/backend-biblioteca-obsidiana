...
function isFixedAdminEmail(email) {
  return FIXED_ADMIN_EMAILS.includes(normEmail(email));
}
...
app.get("/", (req, res) => {
  res.json({ ok:true, name:"Biblioteca Obsidiana Backend V2.2", routes:["/health","/signup","/login","/create-preference","/mercadopago-webhook"] });
});
app.get("/health", (req, res) => res.json({ ok:true, status:"online", version:"2.2" }));
...
app.listen(port, () => console.log(`Biblioteca Obsidiana V2.2 online na porta ${port}`));
