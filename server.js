// server.js — Biblioteca Obsidiana Backend V2.0
// Render Node/Express + Supabase + Mercado Pago
// Implementa:
// - 2 admins fixos vitalícios.
// - inclusão de novos admins.
// - 100 primeiros usuários com 30 dias grátis.
...
app.get("/", (req, res) => {
  res.json({ ok:true, name:"Biblioteca Obsidiana Backend V2.0", routes:["/health","/signup","/login","/create-preference","/mercadopago-webhook"] });
});
app.get("/health", (req, res) => res.json({ ok:true, status:"online", version:"2.0" }));
...
app.post("/create-preference", async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ ok:false, error:"Erro ao criar pagamento." });
  }
});
...
app.post("/mercadopago-webhook", async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ ok:false, error:"Erro no webhook." });
  }
});
...
app.post("/admin/renew-user", requireAdmin, async (req, res) => {
  try {
    ...
    if (current.is_fixed_admin) return res.status(400).json({ ok:false, error:"Admin fixo já é vitalício." });
...
app.post("/admin/block-user", requireAdmin, async (req, res) => {
  ...
  if (current?.is_fixed_admin) return res.status(400).json({ ok:false, error:"Não é possível bloquear admin fixo." });
...
app.post("/admin/delete-user", requireAdmin, async (req, res) => {
  const email = normEmail(req.body.email);
  const { data: current } = await supabase.from("bo_profiles").select("*").eq("email", email).single();
  if (current?.is_fixed_admin) return res.status(400).json({ ok:false, error:"Não é possível deletar admin fixo." });
...
app.listen(port, () => console.log(`Biblioteca Obsidiana V2 online na porta ${port}`));
