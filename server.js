// server.js — Biblioteca Obsidiana Backend V2.3
...
function getPlanPrice(days){ days = Number(days || 30); if(days === 90) return 49.90; if(days === 60) return 34.90; return 19.90; }
...
async function requireAuth(req, res, next){
  ...
  const { data: profile, error } = await supabase.from("bo_profiles").select("*").eq("email", normEmail(data.email)).single();
...
app.get("/", (req, res) => res.json({ ok:true, name:"Biblioteca Obsidiana Backend V2.3" }));
app.get("/health", (req, res) => res.json({ ok:true, status:"online", version:"2.3" }));

app.get("/public/free-slots", async (req, res) => {
  const { count, error } = await supabase.from("bo_profiles").select("*", { count:"exact", head:true }).eq("free_trial_granted", true);
...
app.post("/signup", async (req, res) => {
  try{
    ...
    const profile = {
      email, whatsapp, pin_hash: hashPin(pin), role:"user",
      status: freeTrialGranted ? "active" : "pending_payment",
      is_fixed_admin:false, is_lifetime:false,
    ...
    res.json({ ok:true, profile:data, token, freeTrialGranted, freeTrialNumber, freeRemaining: Math.max(0, 100 - (freeUsed + (freeTrialGranted ? 1 : 0))) });
  }catch(error){
    console.error(error);
...
app.get("/me", requireAuth, (req, res) => res.json({ ok:true, profile:req.profile }));
...
app.post("/create-preference", async (req, res) => {
  try{
    ...
    if(!currentBeforePay){
      await supabase.from("bo_profiles").insert({ email:userEmail, role:"user", status:"pending_payment", free_trial_granted:false, plan_days:0, updated_at:new Date().toISOString() });
    }else if(currentBeforePay.role !== "admin" && !currentBeforePay.is_lifetime){
      await supabase.from("bo_profiles").update({ status:"pending_payment", updated_at:new Date().toISOString() }).eq("email", userEmail);
    }

    const preference = new Preference(mpClient);
    const result = await preference.create({
      body: {
        items: [{ title:`Biblioteca Obsidiana Premium - ${planDays} dias`, quantity:1, currency_id:"BRL", unit_price:price }],
        payer: { email:userEmail },
        metadata: { user_email:userEmail, plan_days:planDays },
        back_urls: { success:`${siteUrl}?payment=success`, failure:`${siteUrl}?payment=failure`, pending:`${siteUrl}?payment=pending` },
        notification_url: `${backendUrl}/mercadopago-webhook`,
        auto_return: "approved"
      }
    });
    res.json({ ok:true, preference_id:result.id, init_point:result.init_point, sandbox_init_point:result.sandbox_init_point });
  }catch(error){
    console.error(error);
...
app.post("/mercadopago-webhook", async (req, res) => {
  try{
    ...
    if(info.status !== "approved") return res.status(200).json({ ok:true, payment_id:paymentId, status:info.status });
    ...
    if(error) throw error;
    res.json({ ok:true, message:"Acesso liberado/renovado.", email:userEmail, expires_at:expiresAt });
  }catch(error){
    console.error(error);
...
app.post("/admin/create-user", requireAdmin, async (req, res) => {
  try{
    ...
    const base = current?.expires_at && new Date(current.expires_at).getTime() > Date.now() ? new Date(current.expires_at) : new Date();
    const payload = {
      email, whatsapp: whatsapp || current?.whatsapp || "",
      role: current?.role || "user",
      status:"active",
      is_lifetime: current?.is_lifetime || false,
      plan_days:days,
...
app.post("/admin/add-admin", requireAdmin, async (req, res) => {
  try{
    ...
    const { data, error } = await supabase.from("bo_profiles").upsert({
      email, pin_hash:hashPin(pin), role:"admin", status:"active",
      is_fixed_admin:isFixed, is_lifetime:true, expires_at:"2099-12-31T23:59:59.000Z",
      created_by:req.profile.email, updated_at:new Date().toISOString()
...
app.post("/admin/block-user", requireAdmin, async (req, res) => {
  ...
  if(current?.is_fixed_admin || isFixedAdminEmail(email)) return res.status(400).json({ ok:false, error:"Não é permitido bloquear administrador fixo." });
  const { data, error } = await supabase.from("bo_profiles").update({ status:"blocked", updated_at:new Date().toISOString() }).eq("email", email).select("*").single();
  if(error) return res.status(500).json({ ok:false, error:error.message });
  res.json({ ok:true, profile:data });
});
...
app.post("/admin/unblock-user", requireAdmin, async (req, res) => {
  const email = normEmail(req.body.email);
  const { data, error } = await supabase.from("bo_profiles").update({ status:"active", updated_at:new Date().toISOString() }).eq("email", email).select("*").single();
  if(error) return res.status(500).json({ ok:false, error:error.message });
  res.json({ ok:true, profile:data });
});
...
app.post("/admin/delete-user", requireAdmin, async (req, res) => {
  const email = normEmail(req.body.email);
  const { data: current } = await supabase.from("bo_profiles").select("*").eq("email", email).single();
  if(current?.is_fixed_admin || isFixedAdminEmail(email)) return res.status(400).json({ ok:false, error:"Não é permitido deletar administrador fixo." });
...
app.listen(port, () => console.log(`Biblioteca Obsidiana V2.3 online na porta ${port}`));
